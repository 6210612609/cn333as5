...
class DbMapper {
    ...
    fun mapContact(contactDbModel: ContactDbModel, tagDbModel: TagDbModel): ContactModel {
        ...
        return with(contactDbModel) { ContactModel(id, name, content, isCheckedOff,  isRecent, tag,) }
    }
    ...
    fun mapDbContact(contact: ContactModel): ContactDbModel =
        with(contact) {
            ...
            if (id == NEW_CONTACT_ID)
                ...
                ContactDbModel(id, name, content, canBeCheckedOff, isCheckedOff, tag.id, false, isRecent)
        }
}